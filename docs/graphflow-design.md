...
**生产就绪版本**：Core 层 P1-P8 + Agent 层 A1-A9 + 文档 D1-D6 + 完整示例 |
...
7. **LLM 接口设计**：LLM 接口应该多简单？是否支持流式调用？是否支持 Function Calling？

8. **MCP 协议支持**：是否在 Agent 层内置 MCP Client/Server 支持？

9. **A2A 协议支持**：是否支持跨语言 Agent 协作？（参考 Google ADK 的 A2A 协议）
