...
- AI Agent 编排通过上层项目 [Cangjie（仓颉）](https://github.com/wzhongyou/cangjie) 实现，Weave 作为其图引擎底座

**设计原则**：

- **分层解耦**：核心引擎与上层应用（如 Cangjie）通过 Go 包依赖解耦，上层应用 import weave，反之不行
...
| Span | 父 Span | 属性 |
|------|---------|------|
| `weave.graph.<name>` | 传入 ctx 的 span | graph.name |
| `weave.node.<name>` | 图 span | node.name, attempt |
| `weave.checkpoint.save` | 图 span | checkpoint.id |
| `weave.checkpoint.load` | 图 span | checkpoint.id |

Metrics：
- `weave.node.duration_ms` (Histogram) — 节点执行耗时
- `weave.node.executions` (Counter) — 节点执行次数，按 status 分
- `weave.graph.duration_ms` (Histogram) — 全图执行耗时
...
```
weave/
├── go.mod                    # module github.com/wzhongyou/weave
│
...
> **Agent 层已迁出** — LLM 节点、Tool 系统、ReAct/RAG/Supervisor Agent 模式等已迁移至 [Cangjie（仓颉）](https://github.com/wzhongyou/cangjie) 项目。Cangjie 依赖 Weave 作为图引擎底座。
...
已从 Weave 迁出至独立项目 [**Cangjie（仓颉）**](https://github.com/wzhongyou/cangjie)。

```
Weave                 Cangjie
─────────                 ───────
图执行引擎（本仓库）  ←──  AI 编程助手（import weave）
...
Cangjie 的 `agent` 包提供所有 AI 相关抽象，通过 `BuildGraph()` 生成 Weave 图，由 Weave 引擎驱动执行。详细的 Agent 层架构、API、使用示例见 [Cangjie 仓库](https://github.com/wzhongyou/cangjie)。
...
**注意**：这只是示例代码，展示如何使用 Weave 构建 Server。Weave 本身不提供正式的 Server 实现。

### 14.3 部署选项

| 选项 | 说明 | 适用场景 |
|------|------|---------|
| **嵌入微服务** | Weave 作为库嵌入到现有服务 | Go 微服务、内部系统 |
...
- 作者：wangzhongyou
- 仓库：https://github.com/wzhongyou/weave
- 文档：https://github.com/wzhongyou/weave/docs
- 讨论：https://github.com/wzhongyou/weave/discussions
